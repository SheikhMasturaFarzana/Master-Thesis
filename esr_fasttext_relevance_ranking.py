...
esr_corpus = esr_corpus_dict['corpus']

corpus_list = [doc['_key'] for doc in esr_corpus]

with open(".\data_docs\esr_pidf_ranks.json", encoding='utf-8') as json_file:
    esr_rank_corpus =  json.load(json_file)

esr_rank = [doc for doc in esr_rank_corpus if doc.get('key') in corpus_list]

thresholds = [ 0.25,0.30, 0.35, 0.40, 0.50, 0.60]
for threshold in thresholds: 
    
    words = ["deep-learning", "question-answering", "computer-vision", "information-geometry", "cryptography"]
    similarity_dict = get_similarity(esr_corpus,[word for word in words], threshold, 'nb_vec_ft') 

    p1_denom, p2_denom = get_global_average(esr_corpus, similarity_dict)
    ...
    for rank_threshold in rank_thresholds:
        ...
        for doc in similarity_dict:
            ...
            rank_dict = dict(next((item for item in esr_rank if item["key"] == doc.get('_key')), 0).get('rank',0))
            doc_words = (next((item for item in esr_corpus if item["_key"] == doc.get('_key')), 0).get('searchtext',0)).split()
        ...
        for word in word_dicts:
            word_list = esr_corpus_dict.get(str(word.replace('-','_').lower()+'_list'))
        ...
        df.to_excel("./result/relevance_ranking/esr_nb_ft_threshold_eval/esr_nb_ft_"+str(threshold)+"_"+str(rank_threshold)+".xlsx", index=False)   
